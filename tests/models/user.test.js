    ...
    describe('static methods', () => {
        ...
        describe('createNewUser', () => {
            ...
            it('should error if we create a new user with an invalid email', async () => {
                ...
                expect(errorObj.message).toEqual('Not a valid email address, sorry.');
            ...
            it('should error if we do not pass an email ', async () => {
                ...
                expect(error.errors.length).toEqual(1);
                const errorObj = error.errors[0];
                expect(errorObj.message).toEqual('Email is required, motherfucker.');
                expect(errorObj.path).toEqual('email');
            });

            it('should error if we create a new user with aan invalid username', async () => {
                const { User } = models;
                const data = {
                    email: 'test@example.com',
                    password: 'Test123#',
